...
function get_tone_from_beat(beat, mod = 1) {
  ...
  if(beat.includes("t")) return {rest: (beat.includes("r") ? true : false), length: length / 10, duration: duration, repeat: 2 * beats * beat.match(/t/g).length};
  return {rest: (beat.includes("r") || (beat[0] == "~" && !beat.includes("t")) ? true : false), length: length, duration: duration, repeat: 1};
}
